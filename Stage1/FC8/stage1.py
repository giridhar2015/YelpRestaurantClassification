...
def image_feature_extract(images, layer = 'fc8'):
...
#store test images features to trainedimages_fc8_features.h5
trainedImagesFeaturesFile = h5py.File(data_root+'trainedimages_fc8_features.h5','w')
...
for i in range(0, size_trainImages, batch_size): 
    ...
    features = image_feature_extract(images, layer='fc8')
    done_num = i+features.shape[0]

    #file resizing happens here to accomodate new batch features 
    features_file= h5py.File(data_root+'trainedimages_fc8_features.h5','r+')
...
#store test images features to testImages_fc8features.h5
testImagesFeaturesFiles = h5py.File(data_root+'testImages_fc8features.h5','w')
...
for i in range(0, size_testImages, batch_size): 
    ...
    #file resizing happens here to accomodate new batch features    
    features_file= h5py.File(data_root+'testImages_fc8features.h5','r+')
    features_file['photo_id'].resize((done_num,))
    features_file['photo_id'][i: done_num] = np.array(pimages)
    features_file['feature'].resize((done_num,features.shape[1]))
    features_file['feature'][i: done_num, :] = features
